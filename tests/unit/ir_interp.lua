...
t.test("provider_eq matches the candidate's provider; not()/or exclude or restrict", function()
    t.truthy(ev({ "provider_eq", "p1" })(cand(), ctx()), "p1 == p1")
    local ok, why = ev({ "provider_eq", "p9" })(cand(), ctx())
    t.falsy(ok); t.contains(why, "provider_id")
    -- "drop antseed" rests on not(provider_eq); "only these N" on or(provider_eq)
    local without = ev({ "not", { "provider_eq", "p1" } })
    t.falsy(without(cand(), ctx()), "p1 is excluded")
    t.truthy(without(cand({ provider_id = "p2" }), ctx()), "p2 survives the exclusion")
    local only = ev({ "or", { "provider_eq", "p2" }, { "provider_eq", "p3" } })
    t.truthy(only(cand({ provider_id = "p3" }), ctx()), "p3 is in the allowed set")
