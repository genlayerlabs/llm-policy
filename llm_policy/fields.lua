...
local function ema_of(cand, ctx)
    ...
    -- keyed per seller peer for marketplace candidates, provider|family for
    -- static ones — the decision lives in util.cand_peer (single source).
    return ema and ema[pm_key(cand.provider_id, cand.model_family, util.cand_peer(cand))] or nil
end
