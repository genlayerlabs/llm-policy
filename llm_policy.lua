...
local function build_filter(spec)
    ...
    if type(spec) == "table" then
        ...
        if spec.price_max then
            -- Prices live in the EMA (seeded from metrics, overwritten by live
            -- traffic), keyed by (provider, model_family) — candidates carry no
            -- price_in/price_out fields (same lookup as R.cost). Missing
            -- metrics PASS: an unknown-price candidate is not filtered out.
            local pin, pout = spec.price_max.input, spec.price_max.output
            return F.where(function(c, ctx)
                local m = ctx.state.ema[pm_key(c.provider_id, c.model_family)]
                if m == nil then return true end
                if pin  ~= nil and m.price_in  ~= nil and m.price_in  > pin  then
                    return false, "price_max"
                end
                if pout ~= nil and m.price_out ~= nil and m.price_out > pout then
                    return false, "price_max"
                end
                return true
            end)
...
-- requests opens every breaker and the next valid request gets no_candidates.
-- Provider-fault kinds (timeout, server_error, network_error, rate_limit,
-- auth_error, bad_response, unknown) keep counting.
